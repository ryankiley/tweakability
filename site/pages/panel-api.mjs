...
export const examples = [
  ...
  },
  {
    ...
    target: `
      <div class="pa-remote">
        <div class="pa-undo-card"></div>
        <div class="pa-remote-row">
          <button class="pa-btn pa-undo-btn" type="button">⌘Z undo</button>
          <button class="pa-btn pa-redo-btn" type="button">⇧⌘Z redo</button>
        </div>
      </div>`,
    css: `
      .pa-undo-card { width: 160px; height: 100px; border-radius: 18px; background: #7C5CFF;
                      transition: width 0.15s, border-radius 0.15s; }`,
    run: ({ tweaks, mount, target }) => {
      const card = target.querySelector(".pa-undo-card");
      ...
      target.querySelector(".pa-undo-btn").addEventListener("click", () => panel.undo());
      target.querySelector(".pa-redo-btn").addEventListener("click", () => panel.redo());
    },
  },
  {
    id: "presets",
    ...
    target: `
      <div class="pa-remote">
        <div class="pa-presets-tile"></div>
        <div class="pa-remote-row">
          <button class="pa-btn pa-save" type="button">savePreset("mine")</button>
          <button class="pa-btn pa-load" type="button">loadPreset("mine")</button>
        </div>
        <div class="pa-presets-note">&nbsp;</div>
      </div>`,
    css: `
    ...
    run: ({ tweaks, mount, target }) => {
      ...
      target.querySelector(".pa-save").addEventListener("click", () => {
        panel.savePreset("mine");
        note.textContent = `presets: ${panel.presets().join(", ")}`;
      });
      target.querySelector(".pa-load").addEventListener("click", () => {
  ...
  },
  {
    ...
    target: `<button class="pa-btn pa-spawn" type="button">Spawn a floating panel</button>`,
    noMount: true,
    run: ({ tweaks, target }) => {
      const btn = target.querySelector(".pa-spawn");
      let floater = null;
